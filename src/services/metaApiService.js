...
function normalizeInsight(insight, adsIndex = {}) {
  ...
  const resultadosUnicos = linkClicksUnique
    ? parseInt(linkClicksUnique.value, 10)
    : parseInt(insight.unique_clicks || 0, 10);
  const custoResultado = linkCost ? parseFloat(linkCost.value) : parseFloat(insight.cpc || 0);

  const outboundClicks = (insight.outbound_clicks || []).reduce((s, a) => s + parseInt(a.value || 0, 10), 0);
...
export async function fetchMetaAgeGender(datePreset = "last_30d") {
  ...
  const results = await Promise.allSettled(AD_ACCOUNT_IDS.map((id) => fetchAgeGenderBreakdown(id, datePreset)));
...
export async function fetchMetaRegion(datePreset = "last_30d") {
  ...
  const results = await Promise.allSettled(AD_ACCOUNT_IDS.map((id) => fetchRegionBreakdown(id, datePreset)));
...
export async function fetchMetaPlatform(datePreset = "last_30d") {
  ...
  const results = await Promise.allSettled(AD_ACCOUNT_IDS.map((id) => fetchPlatformBreakdown(id, datePreset)));
...
export async function fetchMetaDevice(datePreset = "last_30d") {
  ...
  const results = await Promise.allSettled(AD_ACCOUNT_IDS.map((id) => fetchDeviceBreakdown(id, datePreset)));
...
export async function fetchMetaPlacement(datePreset = "last_30d") {
  ...
  const results = await Promise.allSettled(AD_ACCOUNT_IDS.map((id) => fetchPlacementBreakdown(id, datePreset)));
