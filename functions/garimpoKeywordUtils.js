/** Utilitários de termo de busca e relevância para garimpo contextual. */
...
/**
 * Gera termos de busca priorizados a partir do nome/apelido do produto.
 * @returns {{ primario: string, alternativos: string[], tokens: string[] }}
 */
function extrairTermosBuscaGarimpo(nome, apelido) {
...
/**
 * Score 0–100 de similaridade entre produto principal e candidato.
 */
function scoreRelevancia(nomePrincipal, nomeCandidato, shopIdPrincipal, shopIdCandidato) {
  const tokensPrincipal = tokenizarNome(nomePrincipal);
  const tokensCandidato = tokenizarNome(nomeCandidato);
  if (tokensPrincipal.length === 0 || tokensCandidato.length === 0) return 0;

  const setPrincipal = new Set(tokensPrincipal);
  let matches = 0;
  for (const t of tokensCandidato) {
    if (setPrincipal.has(t)) matches += 1;
  }

  let score = (matches / tokensPrincipal.length) * 55;

  for (const p of tokensPrincipal) {
    if (p.length < 4) continue;
    for (const c of tokensCandidato) {
      if (c.includes(p) || p.includes(c)) score += 6;
    }
  }

  if (shopIdPrincipal && String(shopIdPrincipal) === String(shopIdCandidato)) {
    score += 22;
  }

  const tipoPrincipal = tokensPrincipal.find((t) => TIPOS_PRODUTO.has(t));
  const tipoCandidato = tokensCandidato.find((t) => TIPOS_PRODUTO.has(t));
  if (tipoPrincipal && tipoCandidato && tipoPrincipal === tipoCandidato) {
    score += 12;
  }
...
function comissaoR$De(preco, pct) {
  return (Number(preco || 0) * Number(pct || 0)) / 100;
}
