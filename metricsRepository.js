...
export async function getDashboardData(settings = {}) {
  ...
  const [produtos, metaAds, pinterest, cliquesData] = await Promise.all([
    getProdutos(latestShopeeVendaImport?.id || null),
    getMetaAds(latestMetaImport?.id || null),
    getPinterest(latestPinterestImport?.id || null),
    getCliques(latestShopeeCliqueImport?.id || null),
  ]);
  ...
  const effectiveSubIdVendas = (() => {
    ...
    if (Array.isArray(resumo) && resumo.length > 0) return resumo;
  ...
  // ── Índices por subid — espelha o merge outer do dashboard_completo.py ──
  // ads.groupby("subid").sum() → merge com vendas por "subid" → merge com cliques por "subid"
  const metaBySubId = {};
  metaAds.forEach((m) => {
  ...
  // ── Índices por Firestore doc ID (vinculação manual / XLSX histórico) ──
  ...
  // Prioridade 1: metaAdIds (doc IDs) — vinculação manual ou XLSX importado
  // Prioridade 2: subid match — API automática (mesmo comportamento do Python)
  // Isso resolve o problema de metaAdIds ficarem obsoletos após API sync
  const enriched = produtos.map((p) => {
    ...
    if (investimentoFromIds === 0) {
      ...
      subIds.forEach((sid) => {
        ...
        investimentoFromSubId += (metaBySubId[norm]?.gasto || 0)
                               + (pinBySubId[norm]?.gasto  || 0);
      });
    }
  ...
  // metaBySubId e pinBySubId já construídos acima — reutilizados aqui para a tabela SubID
  ...
  const faturamentoBruto  = enriched.reduce((s, p) => s + (p.gmv_total          || 0), 0);
